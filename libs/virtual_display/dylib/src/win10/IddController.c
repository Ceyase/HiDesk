...
char g_lastMsg[1024];
const char* g_msgHeader = "HiDeskIdd: ";
...
BOOL InstallUpdate(LPCWSTR fullInfPath, PBOOL rebootRequired)
{
    ...
    if (FALSE == UpdateDriverForPlugAndPlayDevicesW(
        NULL,
        L"HiDeskIddDriver",    // match hardware id in the inf file
        fullInfPath,
        INSTALLFLAG_FORCE
...
BOOL DeviceCreate(PHSWDEVICE hSwDevice)
{
    ...
    PCWSTR description = L"HiDesk Idd Driver";

    // These match the Pnp id's in the inf file so OS will load the driver when the device is created
    PCWSTR instanceId = L"HiDeskIddDriver";
    PCWSTR hardwareIds = L"HiDeskIddDriver\0\0";
    PCWSTR compatibleIds = L"HiDeskIddDriver\0\0";
    ...
    // Create the device
    HRESULT hr = SwDeviceCreate(L"HiDeskIddDriver",
        L"HTREE\\ROOT\\0",
        &createInfo,
        0,
        NULL,
